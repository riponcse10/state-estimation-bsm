...
#DSRC_MessageID,Vehicle_ID,BSM_tmp_ID,transtime,X,Y,Speed,Instant_Acceleration,Heading,
#brakeStatus,brakePressure,hardBraking,transTo,transmission_received_time

col_list = ["DSRC_MessageID", "Vehicle_ID", "BSM_tmp_ID", "X", "Y", "Speed", "Instant_Acceleration", "Heading", "transmission_received_time"]
df = pd.read_csv("RDEStLouisBSMPart1.csv", usecols=col_list)
...
for index, row in df.iterrows():
    if count<400:
    	if row["Vehicle_ID"] == 13:
    	    
            print(row["Vehicle_ID"], row["Speed"], row["BSM_tmp_ID"], row["X"], row["Y"], row["Instant_Acceleration"], row["Heading"], row["transmission_received_time"])
            pos_data.append((row["X"], row["Y"], 2))
            rad = np.deg2rad(row["Heading"])
            v_data.append((row["Speed"]*np.cos(rad), (row["Speed"]*np.sin(rad)), 0))
            t_data.append(row["transmission_received_time"])
    else:
    	break
    count += 1

print(pos_data[0])
position_data = numpy.array(pos_data)
#position_data = arr.T
print(position_data[1])

print(v_data[0])
velocity_data = numpy.array(v_data)
#velocity_data = arr.T
print(velocity_data.shape)

print(t_data[0])
time_data = numpy.array(t_data)
#time_data = arr.T
...
################################################################################################
#with open('RDEStLouisBSMPart1.csv', 'rb') as file:
...
p_est = np.zeros([position_data.shape[0], 3])  # position estimates
v_est = np.zeros([velocity_data.shape[0], 3])  # velocity estimates
q_est = np.zeros([imu_f.data.shape[0], 4])  # orientation estimates as quaternions
p_cov = np.zeros([imu_f.data.shape[0], 9, 9])  # covariance matrices at each timestep

# Set initial values.
#p_est[0] = gt.p[0]
p_est[0] = position_data[0]
#v_est[0] = gt.v[0]
v_est[0] = velocity_data[0]
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    delta_t = time_data[k] - time_data[k - 1]

    # 1. Update state with IMU inputs
    rotation_matrix = Quaternion(*q_est[k-1]).to_mat()

    # 1.1 Linearize the motion model and compute Jacobians
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2 / 2)*(rotation_matrix.dot(imu_f.data[k-1]))
    v_est[k] = v_est[k-1] + delta_t*(rotation_matrix.dot(imu_f.data[k-1]))
